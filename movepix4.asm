...
pix_num dw 3 ;needs to be number of pix-1 because proc apple adds one
pix_place dw (12*80+39)*2,(12*80+40)*2,(12*80+41)*2, 997 dup (0)	;the location of the pixels
apple_place dw (12*80+25)*2	;the current location of the apple
; --------------------------
CODESEG
; -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;no input
...
push bx
	...
	mov si,[bp+14]	;checks if there are apples on the screen	(apple_num)
	mov cx,[si]
	mov si,[bp+12]	;(apple_place)
	ate_apple:
		mov di,[si]	;the location of the apple
		mov ax,[es:di]
		cmp ah,0eh	;the color of a apple
		jz again1
			jmp new	;doesn't print a new apple if there is one on the screen
		again1:
		add si,2
	loop ate_apple
	jmp outt
	...
	mov si,[bp+8]	;delets the other apples (pix_place)
	mov dx,[si]		;the snakes first pixel location (we need to skip the pixel that the snake ate)
	mov si,[bp+14]	;(apple_num)
	mov cx,[si]		;the number of apples 
	mov si,[bp+12]	;(apple_place)
	mov ah,0
	mov al,' '
	del_apple:
		mov di,[si]
		cmp di,dx
		jz skip		;if it is the apple the snake deletd then it will skip it
			mov [es:di],ax
		skip:
		add si,2
	loop del_apple
	...
	mov si,[bp+14]	;adds another apple because it ate an apple (apple_num)
	mov bx,[si]
	inc bx
	mov [si],bx
	
	mov si,[bp+16]	;adds another apple because it ate an apple, if it reaches to a double digit number, it resets (apple_dis)
	mov bl,[si]
	cmp bl,9
	jnz again
		mov bl,-1
	...
	mov si,[bp+14]	;prints and saves the apples (apple_num)
	mov cx,[si]
	mov si,[bp+16]	;(apple_dis)
	mov bl,[si]		;the char the apple needs to be
	add bl,30h
	...
	print_apple:
		...
		mov al,bl	;prints the apple on the screen
		mov ah,0eh
		mov [es:di],ax
...
mov di,(12*80+25)*2
mov al,'1'	;prints the apple on the screen
mov ah,0eh
mov [es:di],ax

push offset apple_dis		;(bp+16)
push offset apple_num		;(bp+14)
